...
def doxygen(
        ...
        mermaid_render_mode = None,
        mermaid_js_url = None,
        mermaidfile_dirs = None,
    ...
    Args:
        ...
        dot_batch_size: The `dot_batch_size` specifies the number of dot graphs Doxygen is allowed to compile in a single invocation of dot.
        ...
        mermaid_path: When using Mermaid diagrams with CLI rendering, the `mermaid_path` tag should be used to specify the directory where the mmdc (Mermaid CLI) executable can be found.
        mermaid_config_file: When using Mermaid diagrams, the `mermaid_config_file` tag can be used to specify a JSON configuration file for the Mermaid CLI tool (mmdc).
        mermaid_render_mode: The `mermaid_render_mode` tag selects how Mermaid diagrams are rendered.
        mermaid_js_url: The `mermaid_js_url` tag specifies the URL to load mermaid.js from when using client-side rendering (the `mermaid_render_mode` is CLIENT_SIDE or AUTO).
        mermaidfile_dirs: The `mermaidfile_dirs` tag can be used to specify one or more directories that contain Mermaid files that are included in the documentation (see the \\mermaidfile command).
    ...
    _add_generic_configuration(configurations, "DOT_BATCH_SIZE", dot_batch_size)
    ...
    _add_generic_configuration(configurations, "MERMAID_PATH", mermaid_path)
    _add_generic_configuration(configurations, "MERMAID_CONFIG_FILE", mermaid_config_file)
    _add_generic_configuration(configurations, "MERMAID_RENDER_MODE", mermaid_render_mode)
    _add_generic_configuration(configurations, "MERMAID_JS_URL", mermaid_js_url)
    _add_generic_configuration(configurations, "MERMAIDFILE_DIRS", mermaidfile_dirs)
